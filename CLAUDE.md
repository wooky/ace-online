...
`./build` is created root-owned by the container; remove it with `docker run --rm -v "$PWD":/src ace-online-build sh -c 'rm -rf /src/build'` rather than host `rm`.
...
1. **Symbol collisions.** Every game's generated `-img.c` defines the same global `appimglib_imagelib`, and games share other global names. So each game is compiled to an OBJECT library, partial-linked (`cc -r`), then run through `objcopy --keep-global-symbol=<game>_main` so only its entry point stays global. Verify with `nm build/gameobj/<game>.o` (should show exactly one uppercase `<game>_main`).
